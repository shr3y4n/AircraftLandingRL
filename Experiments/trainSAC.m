function [agent, train_stats] = trainSAC(mode, num_episodes)
% TRAINSAC Trains a Soft Actor-Critic (SAC) maximum-entropy continuous landing agent.
%
...
%
% Usage:
%   trainSAC()                 % Runs full training (configured episodes from rlConfig)
%   trainSAC("test")           % Runs 3-episode quick smoke test
%   trainSAC("train", 200)     % Runs training for 200 episodes
%   [agent, stats] = trainSAC(...)
%
% Outputs:
%   agent       - Trained SAC agent struct (standalone pure-MATLAB rlEngine)
%   train_stats - Training history struct with rewards, success rates, and losses
%
% Artifacts saved to:
%   Results/Data/checkpoints/agent_sac_final.mat
%   Results/Data/training_stats_sac.mat
%   Results/Figures/sac_training_curves.png
...
fprintf('========================================================================\n');
fprintf('  EXPERIMENT: SAC REINFORCEMENT LEARNING TRAINING (%s MODE)\n', upper(string(mode)));
fprintf('========================================================================\n');
...
[agent, train_stats] = trainAgent(mode, "sac", P, cfg);

%% 4. Save Final Agent Checkpoint
final_chk = fullfile(chk_dir, 'agent_sac_final.mat');
if isstruct(agent) && isfield(agent, 'actor')
    rlEngine('save', agent, final_chk);
else
    save(final_chk, 'agent');
end
fprintf('[SAC Experiment] Final model checkpoint saved to:\n  %s\n', final_chk);

%% 5. Plot and Save Learning Curves
plotTrainingCurves(train_stats, fig_dir, 'sac_training_curves.png');
...
title('SAC Training Reward Progression', 'FontWeight', 'bold');
...
sgtitle('SAC Autonomous Aircraft Landing: Training Convergence', 'FontSize', 13, 'FontWeight', 'bold');

try
    saveas(fig, fullfile(fig_dir, save_name));
    fprintf('[SAC Experiment] Saved training curve plot to:\n  %s\n', fullfile(fig_dir, save_name));
catch ME
    fprintf('[SAC Experiment] Warning saving plot: %s\n', ME.message);
end
end
