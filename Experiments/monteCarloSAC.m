function summary = monteCarloSAC(num_trials, checkpoint_path)
% MONTECARLOSAC Runs a multi-battery Monte Carlo robustness evaluation for the SAC policy.
%
% Tests the trained SAC policy across 4 rigorous flight batteries:
...
%   checkpoint_path - (Optional) Path to specific SAC checkpoint .mat file
%
% Outputs:
%   summary - Struct array containing statistical performance across batteries
%
% Artifacts saved to:
%   Results/Data/monte_carlo_sac.mat
%   Results/Figures/mc_sac_dispersion.png
...
fprintf('========================================================================\n');
fprintf('  EXPERIMENT: SAC POLICY MONTE CARLO ROBUSTNESS (%d TRIALS / BATTERY)\n', num_trials);
fprintf('========================================================================\n');
...
if nargin < 2 || isempty(checkpoint_path)
    ...
    if ~exist(target_chk, 'file')
        ...
        if isempty(chk_files)
            error('monteCarloSAC:NoCheckpoint', ...
                  ['No trained SAC checkpoint found in:\n  %s\n' ...
                   'Please execute trainSAC("train") before running Monte Carlo.'], chk_dir);
        else
    ...
    if ~exist(target_chk, 'file')
        error('monteCarloSAC:FileNotFound', 'Specified checkpoint does not exist: %s', target_chk);
    end
end
...
suites = {
    'SAC / Calm Air',              'none',       false;
    'SAC / Steady Wind & Shear',   'steady',     false;
    'SAC / Gusts & Turbulence',    'gust_turb',  false;
    'SAC / Randomized Initial IC', 'none',       true;
};
...
plotResults('monte_carlo', summary, 'mc_sac_dispersion.png');

%% 5. Save Results
mat_file = fullfile(data_dir, 'monte_carlo_sac.mat');
save(mat_file, 'summary', 'P', 'num_trials');
fprintf('Saved SAC Monte Carlo dataset to:\n  %s\n', mat_file);
fprintf('========================================================================\n\n');
