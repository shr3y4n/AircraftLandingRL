function summary = monteCarloPID(num_trials)
% MONTECARLOPID Runs a multi-battery Monte Carlo robustness evaluation for the PID baseline.
%
% Tests the classical PID landing controller across 4 rigorous flight batteries:
...
%
% Artifacts saved to:
%   Results/Data/monte_carlo_pid.mat
%   Results/Figures/mc_pid_dispersion.png
...
fprintf('========================================================================\n');
fprintf('  EXPERIMENT: CLASSICAL PID MONTE CARLO ROBUSTNESS (%d TRIALS / BATTERY)\n', num_trials);
fprintf('========================================================================\n');

P = aircraftParameters();

%% 2. Define PID Evaluation Batteries
suites = {
    'PID / Calm Air',              'none',       false;
    'PID / Steady Wind & Shear',   'steady',     false;
    'PID / Gusts & Turbulence',    'gust_turb',  false;
    'PID / Randomized Initial IC', 'none',       true;
};
    ...
    for tr = 1:num_trials
        ...
        switch wind_type
            ...
            case 'steady'
                ...
                wind_cfg.V_headwind = 2.0 + rand() * 8.0; % 2 to 10 m/s
        ...
        custom_init = struct();
        if is_random
            custom_init.h0     = P.h0 + (rand() * 2.0 - 1.0) * 20.0;     % +/- 20m
            custom_init.V0     = P.V_app + (rand() * 2.0 - 1.0) * 4.0;  % +/- 4 m/s
            custom_init.x0     = P.x0 + (rand() * 2.0 - 1.0) * 150.0;   % +/- 150m
...
%% 3. Plot & Save Dispersion Figures
fprintf('\nGenerating Monte Carlo dispersion plots...\n');
plotResults('monte_carlo', summary, 'mc_pid_dispersion.png');

%% 4. Save Results
mat_file = fullfile(data_dir, 'monte_carlo_pid.mat');
save(mat_file, 'summary', 'P', 'num_trials');
fprintf('Saved PID Monte Carlo dataset to:\n  %s\n', mat_file);
fprintf('========================================================================\n\n');
