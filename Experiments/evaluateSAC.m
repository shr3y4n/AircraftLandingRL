function varargout = evaluateSAC(checkpoint_path)
% EVALUATESAC Evaluates a trained SAC reinforcement learning landing policy.
%
...
%                     If omitted, automatically loads the latest SAC checkpoint.
%
% Outputs:
...
%
% Artifacts saved to:
%   Results/Data/sac_evaluation_results.mat
%   Results/Figures/sac_trajectory.png
%   Results/Figures/sac_telemetry.png
%   Results/Figures/sac_wind_trajectory.png
...
fprintf('========================================================================\n');
fprintf('  EXPERIMENT: SAC REINFORCEMENT LEARNING POLICY EVALUATION\n');
fprintf('========================================================================\n');
...
if nargin < 1 || isempty(checkpoint_path)
    target_chk = fullfile(chk_dir, 'agent_sac_final.mat');
    if ~exist(target_chk, 'file')
        chk_files = dir(fullfile(chk_dir, 'agent_sac_ep*.mat'));
        if isempty(chk_files)
            error('evaluateSAC:NoCheckpoint', ...
                  ['No trained SAC checkpoint found in:\n  %s\n' ...
                   'Please execute trainSAC("train") or trainSAC("test") before evaluating.'], chk_dir);
        else
    ...
    if ~exist(target_chk, 'file')
        error('evaluateSAC:FileNotFound', 'Specified checkpoint does not exist: %s', target_chk);
    end
end

fprintf('Loading SAC policy from:\n  %s\n\n', target_chk);
agent = rlEngine('load', target_chk);

%% 3. Nominal Flight Evaluation (Calm Air)
fprintf('Evaluating SAC policy under calm air (wind_mode = none)...\n');
wind_nom.mode = 'none';
[m_nom, log_nom] = evaluateAgent(agent, P, wind_nom);

fprintf('\n------------------------------------------------------------------------\n');
fprintf('  NOMINAL SAC LANDING PERFORMANCE\n');
fprintf('------------------------------------------------------------------------\n');
...
fprintf('\nEvaluating SAC policy under atmospheric wind shear (V_headwind = 6.0 m/s)...\n');
wind_shear.mode = 'shear';
wind_shear.V_headwind = 6.0;
[m_wind, log_wind] = evaluateAgent(agent, P, wind_shear);

fprintf('\n------------------------------------------------------------------------\n');
fprintf('  WIND SHEAR SAC LANDING PERFORMANCE\n');
fprintf('------------------------------------------------------------------------\n');
fprintf('  Landing Status:          %s\n', m_wind.status);
...
plotResults('trajectory', log_nom, P, ...
    'SAC Neural Policy Landing Trajectory (Nominal)', 'sac_trajectory.png');
plotResults('time_history', log_nom, P, ...
    'SAC Neural Policy Closed-Loop Flight Telemetry', 'sac_telemetry.png');
plotResults('trajectory', log_wind, P, ...
    'SAC Landing Trajectory under Wind Shear', 'sac_wind_trajectory.png');

%% 6. Save Structured Evaluation Results
eval_file = fullfile(data_dir, 'sac_evaluation_results.mat');
